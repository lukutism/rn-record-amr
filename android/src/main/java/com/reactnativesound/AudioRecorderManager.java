...
class AudioRecorderManager extends ReactContextBaseJavaModule {
  ...
  public void startRecording(Promise promise) {
    ...
    try {
      ...
      logAndRejectPromise(promise, "NO_PERMISSION", "NO_PERMISSION");
      recorder = null;
    }
  }
  ...
  private void startTimer() {
    ...
    timer.scheduleAtFixedRate(new TimerTask() {
      ...
      public void run() {
        ...
        db = Math.max(Math.min(100, db), 50) - 50;
        WritableMap body = Arguments.createMap();
        body.putInt("currentTime", recorderSecondsElapsed);
        body.putInt("currentDecibel", (int) db);//0~50
